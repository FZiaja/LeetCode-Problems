...
Given a positive integer n, the process of squaring n's digits and summing the squares will be referred to as 
'square-summing' the number n. The result of this process will be referred to as the 'square-sum' of n.

Suppose that n is a positive integer with N digits. Then the square-sum of n is at most (81 * N). This is easy 
to see, since n has N digits, and the square of any digit is at most 81.
...
Main Observation: When we start with any positive integer n, and repeatedly replace it with its square-sum, 
exactly one of the following scenarios occurs:
...
Proof: For all integers between 1 and 324, the observation holds as can easily be verified with some simple code. 
Since all positive integers will eventually land in this range, the conclusion follows.
